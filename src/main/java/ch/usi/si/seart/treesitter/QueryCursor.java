 ...
 *
 * <p>The query cursor should not be shared between threads, but can be reused for many query
 * executions.
 *
 * @since 1.0.0
 * @author Ozren Dabić
...
public class QueryCursor extends External implements Iterable<QueryMatch> {
  ...
  private @Getter boolean executed = false;
   ...
   *     is outside the queried node's byte range
   ...
   *     is outside the queried node's position range
   * @since 1.9.0
   */
  ...
  /**
   * Returns an iterator over the query matches, starting from the first {@link QueryMatch}.
  ...
  public @NotNull Iterator<QueryMatch> iterator() {
    ...
    return new Iterator<>() {
      ...
      private final QueryMatch current = nextMatch();
