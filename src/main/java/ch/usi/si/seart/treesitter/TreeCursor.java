 ...
 * It allows you to walk a syntax tree more efficiently than is possible using the {@link Node}
 * traversal functions. It is always on a certain syntax node, and can be moved imperatively to
 * different nodes.
 *
 * @since 1.0.0
...
public class TreeCursor extends External implements Cloneable {
  ...
  /**
   * Get the depth of the cursor's current {@link Node} relative to the original {@link Node} that
   * the cursor was constructed from.
   *
   ...
   *
   * @return the field name of the tree cursor's current node, {@code null} if the current node
   *     doesn't have a field
   */
  public native String getCurrentFieldName();

  /**
   * Get the {@link TreeCursorNode} representation of the {@link Node} that the cursor is currently
   * pointing to.
   *
   * @return the tree cursor's current node
   */
   ...
   * @return true if the cursor successfully moved, and false if no such child was found
   * @throws IllegalArgumentException if {@code offset} is negative
   * @throws ch.usi.si.seart.treesitter.exception.ByteOffsetOutOfBoundsException if {@code offset}
   *     is outside the current node's byte range
   * @since 1.7.0
   */
  public native boolean gotoFirstChild(int offset);

  /**
   * Move the cursor to the first child of its current node that extends beyond the given row-column
   * offset.
   *
   * @param point the starting row-column position of the child
   * @return true if the cursor successfully moved, and false if no such child was found
   * @throws NullPointerException if {@code point} is null
   * @throws ch.usi.si.seart.treesitter.exception.PointOutOfBoundsException if {@code point} is
   ...
   *
   * <p>Note that this method may be slower than {@link #gotoFirstChild()} because it needs to
   ...
   *
   * <p>Note that this method may be slower than {@link #gotoNextSibling()} due to how node
   * positions are stored. In the worst case, this method may need to iterate through all the
   * previous nodes up to the destination.
   *
   * @return true if the cursor successfully moved, and false if there was no previous sibling node
   * @since 1.12.0
   */
   ...
   *
   * @return true if the cursor successfully moved, and false if there was no parent node (the
   *     cursor was already on the root node)
   */
   ...
   * @return true if the cursor successfully moved, and false if it was already at the specified
   *     node
  ...
  /**
   * Iteratively traverse over the parse tree, applying a callback to the nodes before they are
   * visited.
   *
   ...
   * @return true if the cursor successfully moved, and false if it was already located at the same
   *     node
