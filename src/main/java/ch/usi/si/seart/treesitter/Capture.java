...
/**
 * Represents the named capture of a {@link Query}. Captures are used to extract information from
 * syntax trees when a query match occurs. Each instance can be uniquely identified by the {@link
 * Query} it belongs to, along with its ordinal position within the same query.
 *
 * @since 1.7.0
 * @author Ozren Dabić
 * @see Pattern
...
public class Capture {
  ...
  /**
   * Disable this capture, preventing it from returning in matches. This will eliminate any resource
   * usage from the {@link Query} associated with recording the capture.
   *
  ...
  /**
   * Get the capture quantifiers for all {@link Query} patterns. The order of the quantifiers in the
   * returned list corresponds to the {@link Pattern} order in the query.
   *
