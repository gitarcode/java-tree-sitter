...
/**
 * A single node within a syntax {@link Tree}. It tracks its start and end positions in the source
 * code, as well as its relation to other nodes like its parent, siblings and children.
 *
 * @since 1.0.0
...
public class Node implements Iterable<Node> {
   ...
   * @return the child
   * @throws IndexOutOfBoundsException if the index is a negative number or if it is greater or
   *     equal to the total number of children
   */
   ...
   * @throws ch.usi.si.seart.treesitter.exception.ByteOffsetOutOfBoundsException if either argument
   *     is outside of this node's byte range
   * @throws IllegalArgumentException if:
   *     <ul>
   *       <li>{@code startByte} &lt; 0
   *       <li>{@code endByte} &lt; 0
   *       <li>{@code startByte} &gt; {@code endByte}
   *     </ul>
   *
   * @since 1.6.0
   */
   ...
   * @throws IllegalArgumentException if any point coordinates are negative, or if {@code
   *     startPoint} is a position that comes after {@code endPoint}
   * @throws ch.usi.si.seart.treesitter.exception.PointOutOfBoundsException if any of the arguments
   *     is outside of this node's position range
   * @since 1.6.0
   */
   ...
   *
   * @return the child field name, {@code null} if child does not reside in a field
   * @param child the zero-indexed child position
   * @throws IndexOutOfBoundsException if the index is a negative number or if it is greater or
   *     equal to the total number of children
   */
  ...
  /**
   * Get the syntax tree {@link Symbol} associated with this node as it appears in the grammar,
   * ignoring aliases.
   *
   ...
   * @return the child
   * @throws ch.usi.si.seart.treesitter.exception.ByteOffsetOutOfBoundsException if the byte offset
   *     is outside the node's byte range
   */
   ...
   * @throws IndexOutOfBoundsException if the index is a negative number or if it is greater or
   *     equal to the total number of named children
   * @since 1.9.0
   */
   ...
   *
   * @return {@code true} if the node has been edited, or contains one such child in its subtree,
   ...
   * Syntax errors represent parts of the code that could not be incorporated into a valid syntax
   * tree.
   *
   * @return {@code true} if the node is an {@code ERROR}, or contains one such child in its
   *     subtree, {@code false} otherwise
   */
  public native boolean hasError();

  /**
   * Check if this node represents a syntax error. Syntax errors represent parts of the code that
   * could not be incorporated into a valid syntax tree.
   *
   * @return {@code true} if the node is an {@code ERROR}, {@code false} otherwise
   * @since 1.11.0
   */
  public native boolean isError();

  /**
   * Check if the node is an <em>extra</em>. Extra nodes represent things like comments, which are
   * not required by the grammar, but can appear anywhere.
   *
   * @return {@code true} if the node is an extra, {@code false} otherwise
   */
  public native boolean isExtra();

  /**
   * Check if the node is <em>missing</em>. These nodes are inserted by the parser in order to
   * recover from certain kinds of syntax errors.
   *
   * @return {@code true} if the node is {@code MISSING}, {@code false} otherwise
   */
  public native boolean isMissing();

  /**
   * Check if the node is <em>named</em>. Named nodes correspond to named rules in the grammar,
   * whereas anonymous nodes correspond to string literals in the grammar.
   *
   * @return {@code true} if the node is named, {@code false} otherwise
   */
   ...
   *
   * @return {@code true} if {@code id == 0}, {@code false} otherwise
   */
  ...
  /**
   * Create an iterator over the current node's subtree. The subtree is traversed in a depth-first
   * manner. First iterator element is always the current node.
   *
